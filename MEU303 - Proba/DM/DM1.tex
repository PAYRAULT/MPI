...
La fonction de r\'epartition $F(x)$ est croissante, continue \`a droite et born\'ee entre 0 quand $x \to -\infty$ et 1 quand $x \to +\infty$. Prenons $y / 0 < y < 1$, et $x_1$ la plus petite valeur de $x$ tel que $F(x_1) \geq y$. $x_1$ existe car $F(x)$ est continue (?). Soit $x_2$ une valeur de $x$ telle que $F(x_2) \geq y$. Montrosn que $x_2$ ne peut pas \^etre inf\'erieure \`a $x_1$. Si $x_2$ \'etait inf\'erieure \`a $x_1$ alors $F(x_2) \leq F(x_1)$ car la fonction de r\'epartition $F(x)$ est non d\'ecroissante. Mais $x_1$ est la plus petite valeur de $x$ tel que $F(x_1) \geq y$ donc $x_2$ n'existe pas. 

\subsection*{Question 1.1(b)}
Montrons $F(Q_F(y)+\epsilon) \geq y$. La fonction de r\'epartion $F(x)$ est croissante. Donc $\forall x \in \R, \forall \epsilon > 0, F(x + \epsilon) \geq F(x)$. Comme la fonction $Q_F(y)$ est bien d\'efinie on a $\forall y \in ]0,1[, \forall \epsilon > 0, F(Q_F(y) + \epsilon) \geq F(Q_F(y))$.
