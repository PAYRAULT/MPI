...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \draw[<->] (0,-1)--(0,17); % l'axe des ordonnées
  ...
  \draw[scale=1,domain=0:4,smooth,variable=\t,blue] plot ({2*\t)},{1/2*1/2*4*\t*\t)});
...
Int\'egration de la premi\`ere \'equation: $E_p(\rho,\theta) = -\int{F_{\rho}\, d\rho} = -\int{2\rho \sin \theta\, d\rho} = \rho \cos \theta + f(\theta)$.\\
Identification de la fonction $f(\theta)$. 
$$\rho \cos \theta = -\frac{dE_p(\rho,\theta)}{d\theta} = -\frac{d(-\rho \sin \theta + f(\theta)}{dy} = \rho \cos \theta + f'(\theta)$$
Donc $f'(\theta) = 0$ et $f(\theta) = C^{ste}$.\\
